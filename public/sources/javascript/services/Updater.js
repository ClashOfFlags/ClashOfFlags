export default class Updater {
            ...
            if (this.game.physics.arcade.distanceBetween(barrel, player) < barrel.barrel.range1.value) {
                if (this.game.physics.arcade.distanceBetween(barrel, player) < barrel.barrel.range2.value) {
                    if (this.game.physics.arcade.distanceBetween(barrel, player) < barrel.barrel.range3.value) {
                        this.hitPlayer(player, barrel.barrel.range3.power);
                    } else {
                        this.hitPlayer(player, barrel.barrel.range2.power);
                    }
                } else {
                    this.hitPlayer(player, barrel.barrel.range1.power);
                }
            }
        }
    ...
    bulletHitPlayer(bullet, player) {
        if (player !== this.player && player.visible === true && player.alpha === 1) {
            ...
            if (bullet.team.name !== player.team.name) {
                this.hitPlayer(player, bullet.power);
            }
        }
    }
    ...
    hitPlayer(player, damage) {
        player.hitPlayer(player, damage);
        this.network.sendHit(player, damage);
    }
